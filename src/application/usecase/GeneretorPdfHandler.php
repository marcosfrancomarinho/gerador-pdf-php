...
class GeneretorPdfHandler
{
    ...
    {
        $document = Document::create($input->title, $input->content);
        $file =  $this->creatorPdf->save($document);

        $responseGeneretePdf = new DocumentResponseDTO($document->getNameFilePdf(), $file);
